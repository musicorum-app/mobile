...
object MusicorumAlbumEndpoint {
    ...
    @JvmName("fetchAlbums1")
    suspend fun fetchAlbums(albums: List<TagAlbum?>): List<TrackResponse?> {
        if (albums.isEmpty()) return emptyList()
        val albumList: MutableList<RequestAlbum> = mutableListOf()
        albums.forEach { album ->
            album?.let {
                albumList.add(RequestAlbum(album.name.replace("- Single", ""), album.artist.name))
            }
        }
        val res = KtorConfiguration.musicorumClient.post {
            url("/v2/resources/albums")
